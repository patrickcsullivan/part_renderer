...
bitflags! {
    ...
    pub struct BsdfType: u8 {
        ...
        const BSDF_ALL = Self::BSDF_DIFFUSE.bits | Self::BSDF_GLOSSY.bits | Self::BSDF_SPECULAR.bits | Self::BSDF_REFLECTION.bits | Self::BSDF_TRANSMISSION.bits;
    }
}

/// A bidriectional distribution function (BDF) that can be evaluated for a
/// given incident light direction and a given viewing direction.
pub trait Bdf {
    ...
    /// direction, `wo`, due to light arriving at the surface from the incident
    /// light direction, `wi`.
    ///
    ...
    ///   direction.
    /// * The spetrum of light that is scattered in the viewing direction, `wo`
    ///   due to light arriving at the surface from the returned incident light
    ///   direction.
